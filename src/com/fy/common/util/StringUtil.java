...
public class StringUtil {
	...
	/*分离算法
	* 算法:
	* 1. 读取到"时，判断是否是新子段的第一个，即t是否为空,如果不为空，表明该"是个子串的一部分，
	* 		将"添加到子串中，并读取下一个字符；否则，尝试将该文本子串找出来。具体是：查找下一个", ，
	* 		如果找不到，判断本字符是否最后是一个字符，如果不是就判断最后一个字符是否是",如果是，
	* 		说明剩下的部分是一个""文本子串。读取完文本子串后，移动字符位置索引到文本串的下一个。
	* 2. 读取到,将之前的子串记录到list中，并将子串清空，并移动字符位置索引
	* 3. 其他时候，将该字符记录到子串中，并移动字符位置索引
	* 4. 读取完毕后，将最后一个字串记录到list中（最后一个字串都是不好记录到list中）
	* 
	...
	*/
	...
	public static String subString(String begin, String end, String content) {
		...
		if (!(end==null||end.trim().length()==0)) {
			int index_e_temp = content.indexOf(end);
			if (index_e_temp > -1) {
				index_e = index_e_temp;
			} else {
		...
		return content.substring(index_s, index_e);
	}
	
	   public static void main(String[] argv)throws Exception
	   {//,1,2,3""",5","6",,,"10,"
		   String ss[]=split("\"0,\"8,\",1,2,3\"\"\",4\",\"5\",,,\"8,\",\"10aaaaa\"",',');
